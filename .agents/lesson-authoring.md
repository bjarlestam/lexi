...
Use this skill when creating or editing exercise Markdown for this project. Follow the existing `lessons.md` style: frontmatter, chapter headings, then a YAML-ish list of exercise blocks.
...
Use only for translation tasks where the learner builds the translated answer from word tokens. Always include `pool` explicitly; it should contain all answer tokens plus 2-4 plausible distractors.
...
- For `assemble`, use translation prompts only, and keep answers short enough to tap comfortably on mobile unless the goal is sentence practice.
