...
Use only for tasks where the student translates a sentence. The student builds the translated answer from word tokens. The answer must consist of more than one token. Always include `pool` explicitly; it should contain all answer tokens plus 2-4 plausible distractors.
